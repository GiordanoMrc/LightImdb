 ...
 * banco de dados. 
 */
case class Filme(ID_FILME: Int, titulo: String, diretor: String, anoProducao: Int)
...
class FilmeDAO @Inject() (database: Database){
  ...
  def listar = database.withConnection { implicit connection => 
    SQL"SELECT ID_FILME, TITULO, DIRETOR, ANO_PRODUCAO AS anoProducao FROM TB_FILME".as(parser.*)
  }

  def deletar (idFilme: Int) = database.withConnection { implicit connection =>
    val result = SQL("DELETE FROM TB_FILME WHERE ID_FILME = {id}")
      .on('id -> idFilme).executeUpdate()
  }
